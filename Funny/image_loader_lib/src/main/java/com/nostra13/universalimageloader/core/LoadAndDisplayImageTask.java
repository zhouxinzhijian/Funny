...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
	...
	private static final String LOG_CACHE_IMAGE_IN_GIF_MEMORY = "Cache image in gif memory [%s]";
	...
	public void run() {
		...
		try {
			...
			gifDrawable = configuration.gifMemoryCache.get(memoryCacheKey);
			if (bmp == null || bmp.isRecycled() || gifDrawable == null) {
//			    Log.i("tangjian", "first loading url :  "+uri);
                if (uri.endsWith(".gif")) {
				    gifDrawable = tryLoadGifDrawable();
				}else{
				    bmp = tryLoadBitmap();
				}
				if (bmp == null && gifDrawable == null) return; // listener callback already was fired
				...
				if (options.shouldPreProcess()) {
					...
					if(bmp != null){
					    ...
					    gifDrawable = options.getPreProcessor().process(gifDrawable);
					}
					if (bmp == null) {
				...
				if(gifDrawable != null && options.isCacheInMemory()){
				    L.d(LOG_CACHE_IMAGE_IN_GIF_MEMORY, memoryCacheKey);
				    configuration.gifMemoryCache.put(memoryCacheKey, gifDrawable);
				}
			} else {
			...
			}else if (gifDrawable != null && options.shouldPostProcess()){
			    L.d(LOG_POSTPROCESS_IMAGE, memoryCacheKey);
			    gifDrawable = options.getPostProcessor().process(gifDrawable);
			    if (gifDrawable == null) {
			        L.e(ERROR_POST_PROCESSOR_NULL, memoryCacheKey);
			    }
			}
		...
		DisplayBitmapTask displayBitmapTask = new DisplayBitmapTask(bmp, gifDrawable, imageLoadingInfo, engine, loadedFrom);
	...
	private GifDrawable tryLoadGifDrawable() throws TaskCancelledException {
	    ...
	    try {
	        ...
	        if (imageFile != null && imageFile.exists()) {
	            ...
	            drawable = new GifDrawable(imageFile);
	        }
	        if (drawable == null || drawable.getIntrinsicWidth() <= 0 || drawable.getIntrinsicHeight() <= 0) {
	            ...
	            String imageUriForDecoding = uri;
	            if (options.isCacheOnDisk() && tryCacheGifImageOnDisk()) {
	                imageFile = configuration.diskCache.get(uri);
	                L.i("tangjian", "tryCacheGifImageOnDisk   "+uri+  "    imageFile:  "+ (imageFile!=null?imageFile.getName():"imageFile is empty"));
	                if (imageFile != null) {
	                    imageUriForDecoding = Scheme.FILE.wrap(imageFile.getAbsolutePath());
	                }
	            }
	            ...
	            checkTaskNotActual();
	            L.i("tangjian", "imageUriForDecoding  "+imageUriForDecoding+ "    imageFile:"+ (imageFile!=null?imageFile.getName():"imageFile is empty"));
	            if(imageUriForDecoding != null && imageUriForDecoding.startsWith("file://")){
	                imageUriForDecoding = imageUriForDecoding.replace("file://", "");
	            }
	            drawable = new GifDrawable(imageUriForDecoding);
	...
	private boolean tryCacheImageOnDisk() throws TaskCancelledException {
		...
		try {
			...
			L.e(e);
			if(Scheme.ofUri(uri) == Scheme.HTTP || Scheme.ofUri(uri) == Scheme.HTTPS){
			    IoUtils.reportError(uri, e);
			}
			loaded = false;
		}
		return loaded;
	...
	/** @return <b>true</b> - if image was downloaded successfully; <b>false</b> - otherwise */
	private boolean tryCacheGifImageOnDisk() throws TaskCancelledException {
	    L.d(LOG_CACHE_IMAGE_ON_DISK, memoryCacheKey);
	    
	    boolean loaded;
	    try {
	        loaded = downloadImage();  //不对gif做处理
	    } catch (IOException e) {
	        L.e(e);
	        if(Scheme.ofUri(uri) == Scheme.HTTP || Scheme.ofUri(uri) == Scheme.HTTPS){
                IoUtils.reportError(uri, e);
