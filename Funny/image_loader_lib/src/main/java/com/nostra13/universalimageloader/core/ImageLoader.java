...
public class ImageLoader {
	...
	static final String LOG_LOAD_IMAGE_FROM_GIF_MEMORY_CACHE = "Load image from gif memory cache [%s]";
	...
	private Map<Integer, String> mGifUriMaps = new HashMap<Integer, String>();
	...
	public void displayImage(String uri, ImageAware imageAware, DisplayImageOptions options,
		...
		Bitmap bmp = null;
		boolean gifDrawableEnable = true;
		if(uri.endsWith(".gif")){
		    View wrappedView = imageAware.getWrappedView();
		    Integer key = null;
            if (wrappedView != null) {
                key = imageAware.getWrappedView().hashCode();
            }
		    drawable = configuration.gifMemoryCache.get(memoryCacheKey);
		    if(drawable != null && key != null){
		        mGifUriMaps.remove(key);
		        if(mGifUriMaps.containsValue(memoryCacheKey)){
		            gifDrawableEnable = false;
		            clearRecoveryMemoryCacheKey(memoryCacheKey);
		        }
		    }
		    if (key!=null) mGifUriMaps.put(key, memoryCacheKey);
		}else{
		    bmp = configuration.memoryCache.get(memoryCacheKey);
		}
		if (bmp != null && !bmp.isRecycled() || drawable != null && gifDrawableEnable) {
			L.d(LOG_LOAD_IMAGE_FROM_MEMORY_CACHE, memoryCacheKey);
			L.d(LOG_LOAD_IMAGE_FROM_GIF_MEMORY_CACHE, memoryCacheKey);

			if (options.shouldPostProcess()) {
				ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri, imageAware, targetSize, memoryCacheKey,
						options, listener, progressListener, engine.getLockForUri(uri));
				ProcessAndDisplayImageTask displayTask = new ProcessAndDisplayImageTask(engine, bmp, drawable, imageLoadingInfo,
			    ...
			    if(bmp != null){
			        ...
			        options.getDisplayer().display(drawable, imageAware, LoadedFrom.MEMORY_CACHE);
			    }
				listener.onLoadingComplete(uri, imageAware.getWrappedView(), bmp, drawable);
			}
		} else {
	...
	public GifMemoryCache getGifMemoryCache() {
	    checkConfiguration();
	    return configuration.gifMemoryCache;
	}
	...
	public void clearGifMemoryCache() {
	    checkConfiguration();
	    configuration.gifMemoryCache.clear();
	}
	...
	private static class SyncImageLoadingListener extends SimpleImageLoadingListener {
		...
		@Override
		public void onLoadingComplete(String imageUri, View view, Bitmap loadedImage, GifDrawable drawable) {
	...
	public void clearRecoveryMemoryCacheKey(String memoryCacheKey){
	    Set<Entry<Integer, String>> entrySet = mGifUriMaps.entrySet();
	    List<Integer> keys = new ArrayList<Integer>();
	    for(Entry<Integer, String> entry : entrySet){
	        if(memoryCacheKey.equals(entry.getValue())){
